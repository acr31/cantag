...
#ifndef CHECK_ELLIPSE_SAFAEERAD_GUARD
#define CHECK_ELLIPSE_SAFAEERAD_GUARD
...
namespace Cantag {
  ...
  class CANTAG_EXPORT CheckEllipseSafaeeRad : public Function<TL1(ContourEntity),TL1(ShapeEntity<Ellipse>) > {
  public:
      typedef AggregationFunction Aggregator;
  private:
    const EllipseRestrictions& m_restrict;
  public:
    CheckEllipseSafaeeRad(const EllipseRestrictions& restrict) : m_restrict(restrict) {};
    ...
    float CheckEllipseSafaeeRad<AggregationFunction>::eval(const Ellipse& e, float xi, float yi) {
   ...
   */
  template<class AggregationFunction>
  bool CheckEllipseSafaeeRad<AggregationFunction>::operator()(const ContourEntity& c, ShapeEntity<Ellipse>& e_ent) const {
...
#endif//CHECK_ELLIPSE_SAFAEERAD_GUARD
