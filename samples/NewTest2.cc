...
//    IEEE1394ImageSource fs("/dev/video1394",0);
	...
	Image* i = fs.Next();
      ...
      Apply(contour_tree,convex_tree,ConvexHull(tag)); 
      Apply(contour_tree,o3.m_ContourAlgorithm);
      Apply(contour_tree,DistortionCorrection(camera));
      Tree<ShapeEntity<Ellipse> > shape_tree;
      Apply(contour_tree,shape_tree,FitEllipseLS()); 
      Apply(shape_tree,o3.m_ShapeAlgorithm);
      Tree<TransformEntity> transform_tree;
      Apply(shape_tree,transform_tree,TransformEllipseFull());
      Tree<DecodeEntity<34> > decode_tree;
      Apply(transform_tree,decode_tree,Bind(SampleTagCircle<2,17>(tag,camera),m));
      Apply(decode_tree,Decode<TripOriginalCoder<34,2,2> >());
