...
V4LImageSource::V4LImageSource(const char* device, const int channel) : m_handle(-1), m_mmap((uchar*)MAP_FAILED,-1) {
  ...
  if ((m_handle = VideoDevHandle(open(device,O_RDWR))).Get() < 0) {
  ...
  if ((m_mmap = MMapHandle((uchar*)mmap(0,mbuf.size,PROT_READ,MAP_SHARED,m_handle.Get(),0),mbuf.size)).Get() == (uchar*)MAP_FAILED) {
  ...
  for(int i=0;i<m_total_frames;i++) {
    ...
    m_images[i] = Image(m_image_width,m_image_height,(uchar*)mbuf.offsets[i]);
  }
